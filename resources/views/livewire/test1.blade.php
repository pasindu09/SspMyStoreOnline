...
<div class="mt-auto flex justify-between mt-6">
        ...
        @if($lemem->isNotEmpty())
            ...
            @foreach($lemem as $value)
                @if($value['product_id'] == $item->id)
                    @if($value['quantity'] == 0)
            <button wire:click="addToCart({{ $item->id }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Add To Cart</button>
                    @else
            <button wire:click="addToCart({{ $item->id }})" class="bg-red hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Remove From Cart</button>
                    @endif
    ...
    @else
           ...
           @if($lemem->isNotEmpty())
            ...
            <button wire:click="addToCart({{ $item->id }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Add To Cart</button>
                    @else
                    <button wire:click="addToCart({{ $item->id }})" class="bg-red-800 hover:bg-gray-100 text-white font-semibold py-2 px-4 border border-black-100 rounded shadow">Remove From Cart</button>
                    @endif
    ...
    @auth
        ...
        <button wire:click="view({{ $item->id }})" type="submit" class="bg-gray-200 text-gray-700 py-2 px-4 rounded">View</button>
        </form>
    @else
    @endauth
            </div>
